...
test('closeOnderdelenForExMembers skips admin-managed onderdelen', function () {
    $this->artisan('import:sad-members', ['path' => $this->fixturePath])
        ->assertExitCode(0);

    // Member 9002 is an ex-member — mark KO as admin-managed and assign it
    $relatie = Relatie::where('relatie_nummer', 9002)->first();
    $ko = Onderdeel::where('afkorting', 'KO')->first();
    $ko->update(['beheerd_in_admin' => true]);
    ...
    // Re-import — closeOnderdelenForExMembers should skip admin-managed onderdelen
    $this->artisan('import:sad-members', ['path' => $this->fixturePath])
        ->assertExitCode(0);
