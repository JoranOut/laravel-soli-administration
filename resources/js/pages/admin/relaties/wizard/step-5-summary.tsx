...
export default function Step5Summary({
    ...
    const resolveTypeName = (typeId: string) =>
        relatieTypes.find((rt) => rt.id.toString() === typeId)?.naam ?? typeId;
    const resolveOnderdeelName = (id: string) =>
        onderdelen.find((o) => o.id.toString() === id)?.naam ?? id;
    const resolveInstrumentName = (id: number) =>
        instrumentSoorten.find((is) => is.id === id)?.naam ?? String(id);
                            ...
                            {data.lidmaatschappen.length > 0 && (
                                <div>
                                    ...
                                    {data.lidmaatschappen.map((lid, i) => (
                                        ...
                                        >
                                            <p className="font-medium">
                                                {formatDate(lid.lid_sinds)} –{' '}
                                                {formatDate(lid.lid_tot)}
                                            </p>
                                            {lid.reden_vertrek && (
                                                <p className="text-xs text-muted-foreground">
                                                    {t('Reason')}:{' '}
