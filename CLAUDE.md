...
`muziekbeheer` is in that list because it is a real role **in production**, created by hand and present in no seeder. It was removed from the list once on the strength of a repo-wide grep — check `soli_roles` on the server, not the seeder, before deciding a role does not exist.
